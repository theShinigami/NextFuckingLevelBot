...
const Secrets = require("./Secrets");
 ...
 class Scenes {
         ...
         nfl.on('message', (ctx) => {
             ...
             if (isNaN(msg)) {
                 ...
                 this.redditHandler.returnNextLevel(Number(msg), 'nextfuckinglevel', (u) => {
                     if (u.is_video) {
                         ctx.replyWithVideo(
                             u.media.reddit_video.fallback_url.replace("DASH_360", "DASH_240"),
                             this.keyboard.firstRedditCallback(u.title)
                         );
                     } else if (this.redditHandler.isGIF(u)) {
                        let url = u.media.reddit_video_preview.fallback_url;

                         ctx.replyWithVideo(
                             url.replace(url.substring(url.indexOf("DASH_"), url.length), "DASH_240"),
                             this.keyboard.firstRedditCallback(u.title)
                         );
                     } else {
                         ctx.replyWithPhoto(
     ...
     otherScene() {
         ...
         other.enter((ctx) => {
             ctx.reply("Type in the name of the subreddit and how many post you need.\n\nExample: 'blah 10'", this.keyboard.cancelKeyboard());
             ...
             if (splitted.length == 2 && splitted != undefined) {
                this.redditHandler.returnNextLevel(Number(splitted[1]), splitted[0], (u) => {
                    if (u.is_video) {
                       ...
                       let url = u.preview.reddit_video_preview.fallback_url;
